...
let radarUI = { minutes: +localStorage.getItem("radarMin") || 30, dealsOnly: localStorage.getItem("radarDeals") === "1", loading: false, data: null, err: null, ts: 0 };
...
function renderStations() {
  ...
  const defs = [
    ...
    ["radar", "Radar", "i-search", radarUI.data ? `${radarUI.data.deals}` : ""],
  ];
...
function radarRow(r) {
  ...
  const underTxt = under == null ? (r.market ? "no match" : "no comp") : under >= 0 ? `${under}% under` : `${-under}% over`;
  ...
  const pc = r.product ? ` · ${esc(r.product)}` : "";
  return `<a class="radar-row ${r.deal ? "deal" : ""}" href="${esc(r.url)}" target="_blank" rel="noopener">
    ...
    <span class="rd-main"><span class="rd-title">${esc(r.title)}</span><span class="rd-meta dim">${esc(r.condition || "raw")}${tier}${bids}${pc}</span></span>
    <span class="rd-nums"><span class="rd-bid num">${bid}</span><span class="rd-mkt num dim">mkt ${r.market ? money(r.market) : "—"}</span></span>
...
function renderRadarList() {
  ...
  let list = d.rows;
  if (radarUI.dealsOnly) list = list.filter((r) => r.deal);
  if (!list.length) {
    box.innerHTML = `<div class="radar-empty dim">${radarUI.dealsOnly ? "No under-market auctions in this window yet — widen the window or scan again." : "No auctions ending in this window."}</div>`;
    return;
  }
...
async function loadRadar() {
  ...
  radarUI.loading = true; radarUI.err = null; renderRadar();
  try {
    ...
    if (session) {
      const r = await fetch(SUPABASE_URL + "/functions/v1/ebay", {
        ...
        body: JSON.stringify({ action: "radar", minutes: radarUI.minutes }),
      });
      j = await r.json();
    } else {
      // Not signed in (demo/preview): read-only endpoint, real eBay data, small cap.
      const r = await fetch(SUPABASE_URL + "/functions/v1/ebay?radartest=" + Math.min(radarUI.minutes, 60));
      j = await r.json();
    }
...
function renderRadar() {
  ...
  const sub = radarUI.loading ? "Pulling auctions + pricing…"
    : radarUI.err ? `<span class="neg">${esc(radarUI.err)}</span>`
    : d ? `${d.total} ending · ${d.comped} priced · <b class="pos">${d.deals} under market</b>${updated ? ` · updated ${updated}` : ""}`
    : `Hit Scan to pull auctions ending in the next ${radarUI.minutes} minutes.`;
  v.innerHTML = `
    <div class="radar-head">
      <div><h2>Ending-soon radar</h2>
        <p class="dim">Live Pokémon auctions ending soon — each current bid checked against PriceCharting. Green = bidding below market.</p></div>
      <div class="radar-ctrl">
        ...
        <button class="cta rd-scan" id="rd-scan" ${radarUI.loading ? "disabled" : ""}>${radarUI.loading ? "Scanning…" : ico("i-refresh", "s") + " Scan"}</button>
      </div>
    </div>
    <div class="radar-sub">${sub}</div>
    <div id="radar-list"></div>`;
  v.querySelectorAll(".rd-seg").forEach((b) => b.addEventListener("click", () => {
    radarUI.minutes = +b.dataset.min; localStorage.setItem("radarMin", radarUI.minutes); loadRadar();
  }));
  ...
  $("#rd-scan").addEventListener("click", loadRadar);
  renderRadarList();
  if (!radarTimer) radarTimer = setInterval(tickRadar, 1000);
  if (!d && !radarUI.loading && !radarUI.err) loadRadar();   // auto-scan on first open
}
